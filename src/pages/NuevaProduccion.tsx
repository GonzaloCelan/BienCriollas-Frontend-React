import {
  ...
  CircleDollarSign, Clock3, Factory, Gauge, LoaderCircle, PackageCheck,
...
function displayName(value: string) {
  return value.trim().toLocaleLowerCase("es-AR").replace(
    /(^|[\s-])(\p{L})/gu,
...
export default function NuevaProduccion() {
  ...
  return <section className="real-production-page page-transition">
    ...
    <section className="rp-strip" aria-label="Resumen de producciones">
      <div className="rp-strip__body">
        <div className="rp-strip__title"><small>EN NÚMEROS</small><h3>Producciones</h3></div>
        <div className="rp-strip__metric"><small>BORRADORES</small><strong>{counters.draft}</strong></div>
        <div className="rp-strip__metric"><small>FINALIZADAS</small><strong>{counters.finalized}</strong></div>
        <div className="rp-strip__metric"><small>CANCELADAS</small><strong>{counters.canceled}</strong></div>
        <div className="rp-strip__metric"><small>TOTAL PRODUCCIONES</small><strong>{counters.all}</strong></div>
      </div>
    </section>
    ...
    <div className="rp-workspace">
      ...
      <section className="rp-detail" aria-label="Detalle de producción">
        ...
        <AnimatePresence mode="wait" initial={false}>{!detailLoading && detail && <motion.div className="rp-detail-content" key={detail.id} initial={{ opacity: 0, y: 12, filter: "blur(3px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .28 }}>
          <header className="rp-detail-hero"><div className="rp-detail-heading"><span className={`rp-status rp-status--${detail.status.toLowerCase()}`}>{selectedStatus?.label}</span><h3>{displayName(detail.varietyName)}</h3><p>Producción #{detail.id} · {formatDate(detail.productionDate)}</p><div><span>Receta v{detail.recipeVersion}</span><span>{detail.processVersion ? `Proceso v${detail.processVersion}` : "Sin proceso asociado"}</span></div></div>{detail.status === "DRAFT" && <div className="rp-detail-actions"><button type="button" onClick={() => setConfirmAction("CANCEL")}><Trash2 size={15} />Cancelar</button><button className="is-primary" type="button" onClick={() => { if (!realForm.finalUnits.trim()) { setError("Informá las unidades finales antes de finalizar."); return; } setConfirmAction("FINALIZE"); }}><CheckCircle2 size={15} />Finalizar</button></div>}</header>
